/* Autogenerated file. Do not edit manually. */
/* tslint:disable */

import { Contract, Signer, utils } from 'ethers';
import type { Provider } from '@ethersproject/providers';
import type { CexMarket, CexMarketInterface } from '../CexMarket';

const _abi = [
    {
        inputs: [],
        name: 'BadEmaHalfTime',
        type: 'error',
    },
    {
        inputs: [],
        name: 'BadMaxChangeRatioPerSecond',
        type: 'error',
    },
    {
        inputs: [],
        name: 'BadMaxRawTimeDelta',
        type: 'error',
    },
    {
        inputs: [],
        name: 'BadMaxTimeDelta',
        type: 'error',
    },
    {
        inputs: [],
        name: 'CexNoFeeder',
        type: 'error',
    },
    {
        inputs: [],
        name: 'DecimalsExceed',
        type: 'error',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'token',
                type: 'address',
            },
        ],
        name: 'GetErc20DecimalsFailed',
        type: 'error',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'token',
                type: 'address',
            },
        ],
        name: 'GetErc20SymbolFailed',
        type: 'error',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'aggregator',
                type: 'address',
            },
        ],
        name: 'InvalidCexPrice',
        type: 'error',
    },
    {
        inputs: [],
        name: 'LengthMismatch',
        type: 'error',
    },
    {
        inputs: [],
        name: 'NotConverge',
        type: 'error',
    },
    {
        inputs: [],
        name: 'NotGate',
        type: 'error',
    },
    {
        inputs: [],
        name: 'NotSynFuturesV3Admin',
        type: 'error',
    },
    {
        inputs: [],
        name: 'QuoteNotSupported',
        type: 'error',
    },
    {
        inputs: [],
        name: 'WrongStatusToFetchMarkPrice',
        type: 'error',
    },
    {
        anonymous: false,
        inputs: [
            {
                indexed: true,
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
            {
                indexed: false,
                internalType: 'uint32',
                name: 'expiry',
                type: 'uint32',
            },
        ],
        name: 'ClearStates',
        type: 'event',
    },
    {
        anonymous: false,
        inputs: [
            {
                indexed: false,
                internalType: 'uint8',
                name: 'version',
                type: 'uint8',
            },
        ],
        name: 'Initialized',
        type: 'event',
    },
    {
        anonymous: false,
        inputs: [
            {
                indexed: false,
                internalType: 'uint256',
                name: 'compactEmaParam',
                type: 'uint256',
            },
        ],
        name: 'SetCompactEmaParam',
        type: 'event',
    },
    {
        anonymous: false,
        inputs: [
            {
                indexed: true,
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
            {
                components: [
                    {
                        internalType: 'enum FeederType',
                        name: 'ftype',
                        type: 'uint8',
                    },
                    {
                        internalType: 'uint64',
                        name: 'scaler0',
                        type: 'uint64',
                    },
                    {
                        internalType: 'address',
                        name: 'aggregator0',
                        type: 'address',
                    },
                    {
                        internalType: 'uint24',
                        name: 'heartBeat0',
                        type: 'uint24',
                    },
                    {
                        internalType: 'uint64',
                        name: 'scaler1',
                        type: 'uint64',
                    },
                    {
                        internalType: 'address',
                        name: 'aggregator1',
                        type: 'address',
                    },
                    {
                        internalType: 'uint24',
                        name: 'heartBeat1',
                        type: 'uint24',
                    },
                ],
                indexed: false,
                internalType: 'struct PriceFeeder',
                name: 'feeder',
                type: 'tuple',
            },
        ],
        name: 'SetFeeder',
        type: 'event',
    },
    {
        anonymous: false,
        inputs: [
            {
                indexed: true,
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
            {
                indexed: false,
                internalType: 'uint32',
                name: 'expiry',
                type: 'uint32',
            },
            {
                components: [
                    {
                        internalType: 'uint32',
                        name: 'time',
                        type: 'uint32',
                    },
                    {
                        internalType: 'uint224',
                        name: 'raw',
                        type: 'uint224',
                    },
                    {
                        internalType: 'uint32',
                        name: 'initTime',
                        type: 'uint32',
                    },
                    {
                        internalType: 'uint224',
                        name: 'initMark',
                        type: 'uint224',
                    },
                    {
                        internalType: 'uint256',
                        name: 'accumulation',
                        type: 'uint256',
                    },
                ],
                indexed: false,
                internalType: 'struct CexAccState',
                name: 'state',
                type: 'tuple',
            },
        ],
        name: 'UpdateAccState',
        type: 'event',
    },
    {
        anonymous: false,
        inputs: [
            {
                indexed: true,
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
            {
                components: [
                    {
                        internalType: 'uint16',
                        name: 'emaHalfTime',
                        type: 'uint16',
                    },
                    {
                        internalType: 'uint16',
                        name: 'maxTimeDelta',
                        type: 'uint16',
                    },
                    {
                        internalType: 'uint16',
                        name: 'maxRawTimeDelta',
                        type: 'uint16',
                    },
                    {
                        internalType: 'uint16',
                        name: 'maxChangeRatioPerSecond',
                        type: 'uint16',
                    },
                ],
                indexed: false,
                internalType: 'struct EmaParam',
                name: 'emaParam',
                type: 'tuple',
            },
        ],
        name: 'UpdateEmaParam',
        type: 'event',
    },
    {
        anonymous: false,
        inputs: [
            {
                indexed: true,
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
            {
                components: [
                    {
                        internalType: 'uint32',
                        name: 'time',
                        type: 'uint32',
                    },
                    {
                        internalType: 'uint224',
                        name: 'spot',
                        type: 'uint224',
                    },
                    {
                        internalType: 'uint256',
                        name: 'raw',
                        type: 'uint256',
                    },
                ],
                indexed: false,
                internalType: 'struct SpotState',
                name: 'spotState',
                type: 'tuple',
            },
        ],
        name: 'UpdateSpotState',
        type: 'event',
    },
    {
        inputs: [
            {
                internalType: 'uint32',
                name: 'expiry',
                type: 'uint32',
            },
        ],
        name: 'clearStates',
        outputs: [],
        stateMutability: 'nonpayable',
        type: 'function',
    },
    {
        inputs: [],
        name: 'config',
        outputs: [
            {
                internalType: 'address',
                name: '',
                type: 'address',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
        ],
        name: 'feeders',
        outputs: [
            {
                internalType: 'enum FeederType',
                name: 'ftype',
                type: 'uint8',
            },
            {
                internalType: 'uint64',
                name: 'scaler0',
                type: 'uint64',
            },
            {
                internalType: 'address',
                name: 'aggregator0',
                type: 'address',
            },
            {
                internalType: 'uint24',
                name: 'heartBeat0',
                type: 'uint24',
            },
            {
                internalType: 'uint64',
                name: 'scaler1',
                type: 'uint64',
            },
            {
                internalType: 'address',
                name: 'aggregator1',
                type: 'address',
            },
            {
                internalType: 'uint24',
                name: 'heartBeat1',
                type: 'uint24',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [],
        name: 'gate',
        outputs: [
            {
                internalType: 'address',
                name: '',
                type: 'address',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
            {
                internalType: 'uint32',
                name: 'expiry',
                type: 'uint32',
            },
        ],
        name: 'getAccState',
        outputs: [
            {
                components: [
                    {
                        internalType: 'uint32',
                        name: 'time',
                        type: 'uint32',
                    },
                    {
                        internalType: 'uint224',
                        name: 'raw',
                        type: 'uint224',
                    },
                    {
                        internalType: 'uint32',
                        name: 'initTime',
                        type: 'uint32',
                    },
                    {
                        internalType: 'uint224',
                        name: 'initMark',
                        type: 'uint224',
                    },
                    {
                        internalType: 'uint256',
                        name: 'accumulation',
                        type: 'uint256',
                    },
                ],
                internalType: 'struct CexAccState',
                name: '',
                type: 'tuple',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [],
        name: 'getAllInstruments',
        outputs: [
            {
                internalType: 'address[]',
                name: '',
                type: 'address[]',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
            {
                internalType: 'uint32',
                name: 'expiry',
                type: 'uint32',
            },
        ],
        name: 'getBenchmarkPrice',
        outputs: [
            {
                internalType: 'uint256',
                name: 'benchmark',
                type: 'uint256',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [],
        name: 'getCompactEmaParam',
        outputs: [
            {
                internalType: 'uint256',
                name: '',
                type: 'uint256',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
        ],
        name: 'getEmaParam',
        outputs: [
            {
                components: [
                    {
                        internalType: 'uint16',
                        name: 'emaHalfTime',
                        type: 'uint16',
                    },
                    {
                        internalType: 'uint16',
                        name: 'maxTimeDelta',
                        type: 'uint16',
                    },
                    {
                        internalType: 'uint16',
                        name: 'maxRawTimeDelta',
                        type: 'uint16',
                    },
                    {
                        internalType: 'uint16',
                        name: 'maxChangeRatioPerSecond',
                        type: 'uint16',
                    },
                ],
                internalType: 'struct EmaParam',
                name: '',
                type: 'tuple',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
            {
                internalType: 'uint32',
                name: 'expiry',
                type: 'uint32',
            },
            {
                internalType: 'enum Status',
                name: 'status',
                type: 'uint8',
            },
        ],
        name: 'getMarkPrice',
        outputs: [
            {
                internalType: 'uint256',
                name: 'mark',
                type: 'uint256',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [],
        name: 'getMarketType',
        outputs: [
            {
                internalType: 'string',
                name: '',
                type: 'string',
            },
        ],
        stateMutability: 'pure',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
        ],
        name: 'getRawPrice',
        outputs: [
            {
                internalType: 'uint256',
                name: 'raw',
                type: 'uint256',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
        ],
        name: 'getSpotPrice',
        outputs: [
            {
                internalType: 'uint256',
                name: 'spot',
                type: 'uint256',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
        ],
        name: 'getSpotState',
        outputs: [
            {
                components: [
                    {
                        internalType: 'uint32',
                        name: 'time',
                        type: 'uint32',
                    },
                    {
                        internalType: 'uint224',
                        name: 'spot',
                        type: 'uint224',
                    },
                    {
                        internalType: 'uint256',
                        name: 'raw',
                        type: 'uint256',
                    },
                ],
                internalType: 'struct SpotState',
                name: '',
                type: 'tuple',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'uint256',
                name: '_compactEmaParam',
                type: 'uint256',
            },
        ],
        name: 'initialize',
        outputs: [],
        stateMutability: 'nonpayable',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'uint256',
                name: '',
                type: 'uint256',
            },
        ],
        name: 'instruments',
        outputs: [
            {
                internalType: 'address',
                name: '',
                type: 'address',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [],
        name: 'instrumentsLength',
        outputs: [
            {
                internalType: 'uint256',
                name: '',
                type: 'uint256',
            },
        ],
        stateMutability: 'view',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'uint32',
                name: 'expiry',
                type: 'uint32',
            },
            {
                internalType: 'enum Status',
                name: 'status',
                type: 'uint8',
            },
        ],
        name: 'markPrice',
        outputs: [
            {
                internalType: 'uint256',
                name: 'mark',
                type: 'uint256',
            },
        ],
        stateMutability: 'nonpayable',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'address',
                name: 'instrument',
                type: 'address',
            },
            {
                internalType: 'bytes',
                name: 'data',
                type: 'bytes',
            },
        ],
        name: 'prepareInstrument',
        outputs: [
            {
                internalType: 'bytes32',
                name: 'index',
                type: 'bytes32',
            },
            {
                internalType: 'address',
                name: 'base',
                type: 'address',
            },
            {
                internalType: 'bytes',
                name: 'initData',
                type: 'bytes',
            },
        ],
        stateMutability: 'nonpayable',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'uint256',
                name: 'newCompactEmaParam',
                type: 'uint256',
            },
        ],
        name: 'setCompactEmaParam',
        outputs: [],
        stateMutability: 'nonpayable',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'address[]',
                name: 'instruments',
                type: 'address[]',
            },
            {
                components: [
                    {
                        internalType: 'enum FeederType',
                        name: 'ftype',
                        type: 'uint8',
                    },
                    {
                        internalType: 'uint64',
                        name: 'scaler0',
                        type: 'uint64',
                    },
                    {
                        internalType: 'address',
                        name: 'aggregator0',
                        type: 'address',
                    },
                    {
                        internalType: 'uint24',
                        name: 'heartBeat0',
                        type: 'uint24',
                    },
                    {
                        internalType: 'uint64',
                        name: 'scaler1',
                        type: 'uint64',
                    },
                    {
                        internalType: 'address',
                        name: 'aggregator1',
                        type: 'address',
                    },
                    {
                        internalType: 'uint24',
                        name: 'heartBeat1',
                        type: 'uint24',
                    },
                ],
                internalType: 'struct PriceFeeder[]',
                name: 'priceFeeders',
                type: 'tuple[]',
            },
        ],
        name: 'setFeeder',
        outputs: [],
        stateMutability: 'nonpayable',
        type: 'function',
    },
    {
        inputs: [
            {
                internalType: 'enum Leverage',
                name: 'leverage',
                type: 'uint8',
            },
        ],
        name: 'syncEmaParam',
        outputs: [],
        stateMutability: 'nonpayable',
        type: 'function',
    },
] as const;

export class CexMarket__factory {
    static readonly abi = _abi;
    static createInterface(): CexMarketInterface {
        return new utils.Interface(_abi) as CexMarketInterface;
    }
    static connect(address: string, signerOrProvider: Signer | Provider): CexMarket {
        return new Contract(address, _abi, signerOrProvider) as CexMarket;
    }
}
