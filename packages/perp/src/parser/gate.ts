...
import { Context, ContractParser, formatUnits, formatWad } from '@derivation-tech/context';
import { Gate__factory } from '../typechain';
import { ethers } from 'ethers';
import { ParamType } from 'ethers/lib/utils';
import { LogDescription, TransactionDescription } from '@ethersproject/abi';
import { ErrorDescription } from '@ethersproject/abi/lib/interface';
import { isCexMarket, decodeAddParam, decodeParamForDepositAndWithdraw, formatExpiry } from '../utils';
import { MarketType } from '../enum';
import { NATIVE_TOKEN_ADDRESS } from '../constants';

import { formatSqrtPX96, formatTick, formatTimestamp } from './farmat';
    ...
    switch (param.name) {
      case 'arg':
        if (description.name === 'deposit' || description.name === 'withdraw') {
          ...
          const quoteInfo = usingNative ? this.ctx.wrappedNativeToken : await this.ctx.getTokenInfo(args.token);
      ...
      case 'data': {
        // decode launch data
        if (isCexMarket(description.args['mtype'])) {
