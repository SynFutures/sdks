...
export class GateModule implements GateInterface {
  ...
  async withdrawWad(quoteAddr: string, amount: BigNumber, txOptions?: TxOptions): Promise<ethers.PopulatedTransaction>;
  async withdrawWad(
    quoteAddr: string,
    amount: BigNumber,
    txOptions?: TxOptions,
  ...
  async getPendingParams(
    ...
    const rawRet = (await this.context.getMulticall3().callStatic.aggregate(calls, toPopulatedTxOverrides(txOptions)))
      .returnData;
