...
export class InstrumentModule implements InstrumentInterface {
  ...
  adjustMargin(param: AdjustMarginParam, txOptions: TxOptionsWithSigner): Promise<ethers.providers.TransactionReceipt>;
  ...
  async batchCancelLimitOrder(
    ...
    if (param.orderTicks.length <= MAX_CANCEL_ORDER_COUNT) {
      ...
      const unsignedTx = await instrument.populateTransaction.multicall(calldatas, toPopulatedTxOverrides(txOptions));
      return this.context.tx.sendTx(unsignedTx, txOptions);
    }
  }
