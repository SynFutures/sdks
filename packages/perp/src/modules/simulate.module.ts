...
export class SimulateModule implements SimulateInterface {
  ...
  protected async simulateBatchPlace(
    ...
    if (params.targetTicks.length < MIN_BATCH_ORDER_COUNT || params.targetTicks.length > MAX_BATCH_ORDER_COUNT) {
      throw new SimulationError(`Order count should be between ${MIN_BATCH_ORDER_COUNT} and ${MAX_BATCH_ORDER_COUNT}`);
    }
  ...
  protected _simulateMarketOrderByLeverage(
    ...
    return this.simulateTrade(
      ...
      (
        ...
        const postEquity = wdiv(wmul(markPrice, baseSize.mul(sign).add(prePosition.size)).abs(), params.leverage);
    ...
    return {
      ...
      capitalEfficiencyBoost:
        params.alphaWadLower === params.alphaWadUpper
          ? calcBoost(Number.parseFloat(ethers.utils.formatUnits(params.alphaWadLower)), setting.initialMarginRatio)
          : calcAsymmetricBoost(
