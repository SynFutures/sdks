...
export class ObserverModule implements ObserverInterface {
    ...
    for (let i = 0; i < params.length; i += batchSize) {
      ...
      const calls = _params.map((p) => {
        return {
          ...
          callData: observerInterface.encodeFunctionData('getAcc', [p.instrumentAddr, p.expiry, p.traderAddr]),
        };
      });
  ...
  async parseInstrumentData(
    ...
    for (let i = 0; i < rawList.length; i++) {
      ...
      const marketConfig: MarketConfig = this.context.perp.configuration.config.marketConfig[marketType as MarketType]!;
  ...
  async getQuoteTokenInfo(quoteSymbol: string, instrumentAddr: string, overrides?: CallOverrides): Promise<TokenInfo> {
    return (
  ...
  async inspectCexMarketBenchmarkPrice(instrumentIdentifier: InstrumentIdentifier, expiry: number): Promise<BigNumber> {
    const instrumentAddress = await this.context.perp.instrument.computeInstrumentAddress(instrumentIdentifier);
    const market = this.context.perp.contracts.marketContracts[instrumentIdentifier.marketType]?.market as CexMarket;
    let benchmarkPrice;
    try {
  ...
  async getFundFlows(
    ...
    const blockInfo = trimObj(observerInterface.decodeFunctionResult('getVaultBalances', rawRet[quoteAddrs.length])[1]);
  ...
  async getCexRawSpotPrice(instrumentIdentifier: InstrumentIdentifier, overrides?: CallOverrides): Promise<BigNumber> {
    const instrumentAddress = await this.context.perp.instrument.computeInstrumentAddress(instrumentIdentifier);
    const market = this.context.perp.contracts.marketContracts[instrumentIdentifier.marketType]?.market as CexMarket;
    let rawSpotPrice;
    try {
  ...
  async getGateBalances(target: string, overrides?: CallOverrides): Promise<(TokenInfo & { balance: BigNumber })[]> {
    const quotes = await Promise.all(
      Object.keys(this.context.perp.configuration.config.quotesParam).map((quote) => this.context.getTokenInfo(quote)),
    );
