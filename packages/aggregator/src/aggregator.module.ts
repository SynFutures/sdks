...
export class AggregatorModule implements AggregatorInterface {
  ...
  async getPoolList(token0Address: string, token1Address: string, excludePoolTypes: PoolType[] = []): Promise<Pair[]> {
  ...
  async simulateMixSwap(params: SimulateMixSwapParam): Promise<SimulateMixSwapResult> {
    ...
    const minReturnAmount = querySingleRouteResult.bestAmount.mul(RATIO_BASE - params.slippageInBps).div(RATIO_BASE);
